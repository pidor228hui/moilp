...
async def show_ignore_global_members(
    ...
    if not user_ids and not group_ids:
        return "Ты глобально никого не игноришь🤦🏿‍♂️"

    index = 1
    message = "Ты глобально игноришь их:\n"
...
async def add_ignored_global_member_wrapper(
    ...
    if not member_ids:
        await edit_message(
            message,
            f'Кого там в глобальный кинуть?'
        )
        return
    ...
    if member_id == await message.api.user_id:
        await edit_message(
            message,
            f'Опять ты себя хочешь в игнор кинуть?'
        )
        return
    ...
    if member_id in [
        ...
        await edit_message(
            message,
            f'⚠ {name} уже в глоигноре'
        )
        return
    add_ignore_global_member(db, member_id)
    await edit_message(
        message,
        f'✅ {name} Пошел в глобальный игнор...'
    )
...
async def remove_ignored_global_member_wrapper(
    ...
    if member_id not in [
        ...
        await edit_message(
            message,
            f'⚠ {name} не в списке игноров'
        )
        return
    remove_ignore_global_member(db, member_id)
    await edit_message(
        message,
        f'✅ {name} убран из игнора'
    )
